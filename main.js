document.addEventListener("DOMContentLoaded", function() {
    ...
    function drawStars(starPositions, longIndex) {
        ...
        updateMainBottomRow(currentPoint);
        updateProgressBar(8);
        // 중앙 메인 화면에 S$ image 표시
    ...
    function updateScoreDisplay() {
        ...
        if (scoreBar) {
            scoreBar.innerHTML = `point:<span id="point-box">${point}</span> try:<span id="try-box">${tryCount}</span> total:<span id="total-box">${total}</span>`;
        }
    ...
    function updateProgressBar(count = 8) {
    ...
    function updateMainBottomRow(currentPoint) {
        // 1. 왼쪽 원형 아이콘 2개
        const iconBox = document.getElementById('main-bottom-icon');
        iconBox.innerHTML = '';
        for (let i = 1; i <= 2; i++) {
            const icon = currentPoint && currentPoint[`S1 icon_${i}`];
            if (icon) {
                const img = document.createElement('img');
                img.src = 'assets/images/yoso/' + icon;
        ...
        // 2. 파란색 6칸
        const cells = document.getElementById('main-bottom-cells');
        cells.innerHTML = '';
        for (let i = 0; i < 6; i++) {
            const cell = document.createElement('div');
            cell.style.width = '80px';
            ...
            cell.style.background = '#87ceeb';
            cell.style.border = '3px solid #222';
            cell.style.margin = '0 2px';
            cell.style.display = 'inline-block';
        ...
        // 3. 점수/try/total
        const pointScore = document.getElementById('point-score');
        if (pointScore) pointScore.textContent = 'point: ' + point;
        const tryBtn = document.getElementById('try-btn');
        if (tryBtn) tryBtn.textContent = 'try';
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      fetch('assets/data/data_1_3.json')
        ...
        .then(data => {
          loadedData = data;
          const tbody = document.querySelector('#s1004-table tbody');
          tbody.innerHTML = '';
          data.forEach((item, idx) => {
            const points = item.points;
            const s1004 = points[points.length - 1];
            const row = document.createElement('tr');
            row.innerHTML = `
              <td>${item.work_sentence}</td>
              <td>${s1004['S1004 po_sition'] || ''}</td>
              <td>${s1004['S1004 main_category'] || ''}</td>
              <td>${s1004['S1004 sub_category'] || ''}</td>
              <td>${s1004['S1004 special_value'] || ''}</td>
              <td>${s1004['S1004 text'] || ''}</td>
            `;
            row.style.cursor = 'pointer';
            row.addEventListener('click', function() {
              showS1004StarOnImage(item);
            });
            tbody.appendChild(row);
          });
          // 첫 번째 행 자동 표시
          if (data.length > 0) showS1004StarOnImage(data[0]);
        });

      function showS1004StarOnImage(item) {
        const points = item.points;
        const s1004 = points[points.length - 1];
        // 대표 이미지는 S1 image 사용
        const s1 = points[0];
        let imgFile = s1['S1 image'] || '';
        const img = document.getElementById('main-image');
        if (imgFile) {
          // 경로 자동 판별 (assets/images/ 또는 assets/picture/)
          if (imgFile.match(/\.(jpg|png|jpeg|gif|avif)$/i)) {
        ...
        // 별 위치 계산
        const star = document.getElementById('s1004-star');
        ...
        if (pos && pos.match(/\(([^,]+),\s*([^)]+)\)/)) {
          ...
          img.onload = function() {
            ...
            star.style.left = (x * w) + 'px';
            star.style.top = (y * h) + 'px';
